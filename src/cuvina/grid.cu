...
COULD_INLINE void cache_eval_deriv(Cache &c, Model &m) {
	...
	CU_FOR(i, src->movable_atoms) {
		...
		if (t != CU_INVALID_XS_SIZE) {
			...
			e = grid_eval(g, src->coords[i], c.m_slope, src->movable_v, &deriv);
		}
...
GLOBAL  void model_eval_deriv(Model &m, PrecalculateByAtom &p, Cache &c, Change &g) {
	...
	CU_FOR(i, src->npairs) {
		eval_interacting_pair_deriv(i, p, src->pairs[i], src->coords, m.pair_res[i]);
	}
	SYNC();
	...
	CU_FOR(i, src->nligand) {
		...
		FOR(j, ligand.nr_node) {
			CUDBG("ligand %d", j);
			sum_force_and_torque<Segment>(ligand.tree[j], var.tree[j].origin, src->coords, m.minus_forces, var.tree[j].ft);
	...
	CU_FOR(i, src->nflex) {
		...
		FOR(j, flex.nr_node) {
			CUDBG("flex %d", j);
			sum_force_and_torque<Segment>(flex.tree[j], var.tree[j].origin, src->coords, m.minus_forces, var.tree[j].ft);
			var.tree[j].dirty = 0; // once force accumulated, dirty will be set to non-0 and torque should be re-calculated
		}
		// climbing from the leaves to root and accumulate force and torque
		FOR(j, flex.nr_node) {
